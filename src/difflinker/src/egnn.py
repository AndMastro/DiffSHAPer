...
class GCL(nn.Module):
    ...
    def edge_model(self, source, target, edge_attr, edge_mask):
        source = source.to(device=running_device)
        target = target.to(device=running_device)
        edge_attr = edge_attr.to(device=running_device)
        edge_mask = edge_mask.to(device=running_device) #@mastro moved all to device (hopefully the selected GPU)
    ...
    def forward(self, h, edge_index, edge_attr=None, node_attr=None, node_mask=None, edge_mask=None):
        row, col = edge_index
        #move row and col to running device @mastro
        row = row.to(device=running_device)
        col = col.to(device=running_device)
        edge_feat, mij = self.edge_model(h[row], h[col], edge_attr, edge_mask)
        h, agg = self.node_model(h, edge_index, edge_feat, node_attr)
        if node_mask is not None:
            node_mask = node_mask.to(device=running_device) #@mastro moved node_mask to device (hopefully the selected GPU)
            h = h * node_mask
        return h, mij
...
class EquivariantUpdate(nn.Module):
    ...
    def coord_model(self, h, coord, edge_index, coord_diff, edge_attr, edge_mask, linker_mask):
        ...
        coord_diff = coord_diff.to(device=running_device) #@mastro moved coords_range to device (hopefully the selected GPU)
        if self.tanh:
            trans = coord_diff * torch.tanh(self.coord_mlp(input_tensor)) * self.coords_range
        else:
            trans = coord_diff * self.coord_mlp(input_tensor)
        if edge_mask is not None:
            edge_mask = edge_mask.to(device=running_device) #@mastro moved edge_mask to device (hopefully the selected GPU)
            trans = trans * edge_mask
        agg = unsorted_segment_sum(trans, row, num_segments=coord.size(0),
                                   normalization_factor=self.normalization_factor,
                                   aggregation_method=self.aggregation_method)
        if linker_mask is not None:
            linker_mask = linker_mask.to(device=running_device) #@mastro moved linker_mask to device (hopefully the selected GPU)
            agg = agg * linker_mask

        coord = coord.to(device=running_device) #@mastro moved coord to device (hopefully the selected GPU)
        coord = coord + agg
        return coord
...
class EquivariantBlock(nn.Module):
    ...
    def forward(self, h, x, edge_index, node_mask=None, linker_mask=None, edge_mask=None, edge_attr=None):
        ...
        if node_mask is not None:
            node_mask = node_mask.to(device=self.device) #@mastro moved node_mask to device of x (hopefully the selected GPU)
            h = h * node_mask
        return h, x
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', activation=nn.SiLU(), n_layers=3, attention=False,
        ...
        for i in range(0, n_layers):
            self.add_module("e_block_%d" % i, EquivariantBlock(hidden_nf, edge_feat_nf=edge_feat_nf, device=self.device,
...
def coord2diff(x, edge_index, norm_constant=1):
    row, col = edge_index
    row = row.to(device=running_device) #@mastro moved row to device (hopefully the selected GPU)
    col = col.to(device=running_device) #@mastro moved col to device (hopefully the selected GPU)
    x = x.to(device=running_device) #@mastro moved x to device (hopefully the selected GPU)
...
def unsorted_segment_sum(data, segment_ids, num_segments, normalization_factor, aggregation_method: str):
    ...
    segment_ids = segment_ids.to(device=running_device) #@mastro moved segment_ids to device (hopefully the selected GPU)
    data = data.to(device=running_device) #@mastro moved data to device (hopefully the selected GPU)
