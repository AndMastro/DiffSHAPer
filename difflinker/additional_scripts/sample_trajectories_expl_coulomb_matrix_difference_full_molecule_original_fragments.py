...
chains_output_dir = os.path.join(chains, experiment_name, prefix, 'chains_coulomb_difference_' + P + '_seed_' + str(SEED))
final_states_output_dir = os.path.join(chains, experiment_name, prefix, 'final_states_coulomb_difference_' + P + '_seed_' + str(SEED))
...
folder_save_path = "results/explanations_coulomb_difference_" + P + "_seed_" + str(SEED) + "_full_molecule_original_fragments"
...
for data_index, data in enumerate(tqdm(data_list)): #7:
        ...
        # Sort the Coulomb matrix according to its eigenvalues as shown in https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.108.058301
        eigenvalues, eigenvectors = torch.linalg.eigh(coulomb_matrix)
        eigenvalues = torch.abs(eigenvalues)
        sorted_indices = torch.argsort(eigenvalues, descending=True)
        coulomb_matrix = coulomb_matrix[sorted_indices][:, sorted_indices]

        print("Coulomb matrix (sorted according to absolute eigenvalues): ", coulomb_matrix)

        frobenius_norm_original_linker = torch.linalg.norm(coulomb_matrix, ord='fro')

        print("Frobenius norm of the original molecule:", frobenius_norm_original_linker)

        coulomb_matrix_batch = coulomb_matrix.unsqueeze(0).repeat(PARALLEL_STEPS, 1, 1)

        print("Coulomb matrix batch shape:", coulomb_matrix_batch.shape)
        ...
        for j in tqdm(range(num_fragment_atoms)): 
            ...
            for step in range(int(M/PARALLEL_STEPS)):
                ...
                # Sort V_j_plus_coulomb_matrices_batch according to the absolute eigenvalues in decreasing order
                sorted_V_j_plus_coulomb_matrices_batch = []
                for matrix in V_j_plus_coulomb_matrices_batch:
                    eigenvalues, eigenvectors = torch.linalg.eigh(matrix)
                    eigenvalues = torch.abs(eigenvalues)
                    sorted_indices = torch.argsort(eigenvalues, descending=True)
                    sorted_matrix = matrix[sorted_indices][:, sorted_indices]
                    sorted_V_j_plus_coulomb_matrices_batch.append(sorted_matrix)
                V_j_plus_coulomb_matrices_batch = torch.stack(sorted_V_j_plus_coulomb_matrices_batch)
                
                #to use concept from the paper, we need difference of sorted coulomb matrices
                V_j_plus_frobenius_norm_batch = compute_frobenius_norm_batch(coulomb_matrix_batch - V_j_plus_coulomb_matrices_batch)
                ...
                sorted_V_j_minus_coulomb_matrices_batch = []
                for matrix in V_j_minus_coulomb_matrices_batch:
                    eigenvalues, eigenvectors = torch.linalg.eigh(matrix)
                    eigenvalues = torch.abs(eigenvalues)
                    sorted_indices = torch.argsort(eigenvalues, descending=True)
                    sorted_matrix = matrix[sorted_indices][:, sorted_indices]
                    sorted_V_j_minus_coulomb_matrices_batch.append(sorted_matrix)
                V_j_minus_coulomb_matrices_batch = torch.stack(sorted_V_j_minus_coulomb_matrices_batch)

                V_j_minus_frobenius_norm_batch = compute_frobenius_norm_batch(coulomb_matrix_batch - V_j_minus_coulomb_matrices_batch)
                ...
                sorted_V_random_coulomb_matrices_batch = []
                for matrix in V_random_coulomb_matrices_batch:
                    eigenvalues, eigenvectors = torch.linalg.eigh(matrix)
                    eigenvalues = torch.abs(eigenvalues)
                    sorted_indices = torch.argsort(eigenvalues, descending=True)
                    sorted_matrix = matrix[sorted_indices][:, sorted_indices]
                    sorted_V_random_coulomb_matrices_batch.append(sorted_matrix)
                V_random_coulomb_matrices_batch = torch.stack(sorted_V_random_coulomb_matrices_batch)

                V_random_frobenius_norm_batch = compute_frobenius_norm_batch(coulomb_matrix_batch - V_random_coulomb_matrices_batch)
                ...
                mean_V_random_frobenius_norm = torch.linalg.norm(coulomb_matrix - mean_V_random_coulomb_matrix, ord='fro') #this is already the expected values 2
        ...
        with open(f'{folder_save_path}/phi_atoms_{data_index}.txt', 'w') as write_file:
            ...
            expected_value_1 = frobenius_norm_original_linker - average_frobenius_norm_random_samples
            current_prediction = frobenius_norm_original_linker - frobenius_norm_original_linker

            expected_value_2 = mean_V_random_frobenius_norm

            write_file.write("Expected value (Frobenius norm of original molecule - average Frobenius norm of random samples):")
            ...
            write_file.write("Expected value (Frobenius norm of original molecule - Frobenius norm of the average random sample):")
