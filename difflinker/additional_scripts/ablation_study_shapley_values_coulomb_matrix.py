...
INTIAL_DISTIBUTION_PATH = "results/explanations_coulomb_" + P + "_seed_" + str(SEED)
...
for data_index, data in enumerate(tqdm(data_list)):
    ...
    with open(INTIAL_DISTIBUTION_PATH + "/phi_atoms_" + str(data_index) + ".txt", "r") as read_file:
        for row in read_file:
            if row.strip().startswith("Original linker frobenius norm"):
                row = read_file.readline()
                line = row.strip()
                print("line", line)
                line = ''.join(filter(lambda x: x.isdigit() or x == '.', line))
                original_frobenius_norm = float(line)
            if row.strip().startswith("Average frobenius norm random samples"):
                row = read_file.readline()
                line = row.strip()
                #take only numbers and dots from line
                line = ''.join(filter(lambda x: x.isdigit() or x == '.', line))           
                average_frobenius_norm = float(line)    
