...
def create_template(tensor, fragment_size, linker_size, fill=0):
    ...
    values_to_keep = values_to_keep.to(running_device) #@mastro moved to device of values_to_keep
...
def create_templates_for_linker_generation(data, linker_sizes):
    ...
    for i, linker_size in enumerate(linker_sizes):
        data_dict = {}
        fragment_mask = data['fragment_mask'][i].squeeze()#@mastro moved to device of fragment_mask
