...
import time 
from src.utils import remove_mean_with_mask
...
for data_index, data in enumerate(tqdm(data_list)): #7:
        ...
        if SAVE_VISUALIZATION:
            ...
            for i in range(len(data['positions'])):
                ...
                # Create tensors for positions, one hot, and atom mask
                positions_combined = torch.zeros_like(data['positions'])
                one_hot_combined = torch.zeros_like(data['one_hot'])
                # atom_mask_combined = torch.ones_like(node_mask[i])
                ...
                for atom_idx in range(data['positions'].shape[1]):
                    if data['fragment_mask'][0, atom_idx] == 1:
                        ...
                        # atom_mask_combined[:, atom_idx] = node_mask[i, atom_idx]
                #remove mean from comedined positions
                # positions_combined = remove_mean_with_mask(positions_combined, node_mask[i])
                # Save the combined data

                #normalize positions and features

                # print("one hot combined size", one_hot_combined.size())
                # print("node mask[0]", node_mask[i].size())

                # mask = node_mask[i].unsqueeze(0)
                # mask = node_mask[0].squeeze()
                # n_atoms = mask.sum()
                # atom_idx = torch.where(mask)[0]
                # atoms = torch.argmax(one_hot_combined[0], dim=1)
                ...
                # node_mask_unsqueezed = node_mask[i].unsqueeze(0)
                
                # mask = node_mask[0].squeeze()
                # n_atoms = mask.sum()
                # atom_idx = torch.where(mask)[0]
                # print("Mask", mask)
                # print("Atom idx", atom_idx)
                # atoms = torch.argmax(noisy_features_present_atoms.unsqueeze(0)[0], dim=1)
                # print("Atoms", atoms)
                # print("noisy_features_present_atoms", noisy_features_present_atoms.unsqueeze(0))
                # arrestomomentum()
                ...
                #     node_mask_unsqueezed, #FIX che if ok
                ...
                # print("Chain node mask shape", chain_node_mask.shape)
                # print("Chain node mask", chain_node_mask)
                # print("Node mask shape", node_mask[i].unsqueeze(0).shape)
                # print("Node mask", node_mask[i].unsqueeze(0))
                # print("Noisy features shape", noisy_features_present_atoms.shape)
                # print("Noisy positions shape", noisy_positions_present_atoms.shape)
                # arrestomomentum()
